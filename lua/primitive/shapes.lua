...
local function primitive_prebuild(vars, nophys)
	local ret = {}
	for k, v in pairs(vars) do
		ret[string.gsub(k, "_primitive_", "")] = v
	end
	if shapes[ret.shape] then
		return shapes[ret.shape](ret, nophys), ret
	end
end
addon.primitive_prebuild = primitive_prebuild

local function primitive_build(vars, nophys, func)
	local shape, vars = primitive_prebuild(vars, nophys)
	if not shape then
		return false
	end
	local succ, ret
	if func then
		...
		succ, ret = pcall(addon.primitive_triangulate, shape.vertex, shape.index, tonumber(normals))
		if not succ then
			MsgC(Color(255, 255, 0), "primitive addon: ", Color(255, 100, 100), "malformed shape\n")
			MsgC(Color(255, 255, 0), "primitive addon: ", Color(255, 100, 100), ret .. "\n")
			ret = nil
		end
	end
	return shape, ret
