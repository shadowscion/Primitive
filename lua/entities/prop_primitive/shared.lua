...
local safe_kv = {
	_primitive_type = function(self, oldvalue, newvalue)
	...
	_primitive_dx = function(self, oldvalue, newvalue) return math.Clamp(newvalue, 1, 1024) end,
	_primitive_dy = function(self, oldvalue, newvalue) return math.Clamp(newvalue, 1, 1024) end,
	_primitive_dz = function(self, oldvalue, newvalue) return math.Clamp(newvalue, 1, 1024) end,
	_primitive_dt = function(self, oldvalue, newvalue) return math.Clamp(newvalue, 1, 1024) end,
	_primitive_maxseg = function(self, oldvalue, newvalue) return math.Round(math.Clamp(newvalue, 1, 32)) end,
	_primitive_numseg = function(self, oldvalue, newvalue) return math.Round(math.Clamp(newvalue, 1, 32)) end,
	_primitive_numring = function(self, oldvalue, newvalue) return math.Round(math.Clamp(newvalue, 1, 32)) end,
	_primitive_tx = function(self, oldvalue, newvalue) return math.Clamp(newvalue, -1, 1) end,
	_primitive_ty = function(self, oldvalue, newvalue) return math.Clamp(newvalue, -1, 1) end,
}

local mod_patterns = {"(sides=%d%d%d%d%d%d)"}
safe_kv._primitive_modv = function(self, oldvalue, newvalue)
...
function ENT:_primitive_reset(shape)
	...
	shape = typevars._primitive_type
	for k, v in pairs(shape_generic) do
		self["Set" .. k](self, typevars and typevars[k] or v)
	end
end

function ENT:_primitive_setsafe(name, oldvalue, newvalue)
	if oldvalue == newvalue then
		return
	end
	if safe_kv[name] then
		local value = safe_kv[name](self, oldvalue, newvalue)
...
function ENT:SetupDataTables()
	...
	self:NetworkVar("String", 1, "_primitive_modv", {KeyName = "_primitive_modv", Edit = {order = 306, category = category, title = "Variables", type = "String", waitforenter = true}})

	self:NetworkVarNotify("_primitive_type", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_dx", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_dy", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_dz", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_dt", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_maxseg", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_numseg", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_numring", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_tx", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_ty", self._primitive_setsafe)
	self:NetworkVarNotify("_primitive_modv", self._primitive_setsafe)
...
function ENT:Think()
	if self._primitive_trigger_update and CurTime() - self._primitive_trigger_update > (g_primitive.update_delay or 0.015) then
		...
		if hook.Run("primitive_update_pre", self) == false then
			return
		end
